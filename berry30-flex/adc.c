...
#define SAMPLE_PERIOD 9600//4800//2400
		...
		if (int_en)
		{
			...
			if (level && (level > prev_level))
			{
				if (level == 1 && (prev_level < 1))
					registers[INTERRUPT] |= (int_en & ON_BEND_1);
				else if (level == 2 && (prev_level < 2))
					registers[INTERRUPT] |= (int_en & ON_BEND_2);
				else if (level == 3 && (prev_level < 3))
					registers[INTERRUPT] |= (int_en & ON_BEND_3);
			}
			// Released:
			if (prev_level && !level)
				registers[INTERRUPT] |= (int_en & ON_STRAIGHT);
